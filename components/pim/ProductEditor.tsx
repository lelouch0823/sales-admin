  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        ...
        setEditForm({
          sku: '', name: '', brand: '', category: '', price: 0, 
      ...
      <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
              <div>
                  ...
                  <input className="w-full border p-2 rounded text-sm bg-gray-50" value={editForm.sku} onChange={e => setEditForm({...editForm, sku: e.target.value})} disabled={!!initialData} />
              </div>
              <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('pim.editor.brand')}</label>
                  <input className="w-full border p-2 rounded text-sm" value={editForm.brand} onChange={e => setEditForm({...editForm, brand: e.target.value})} />
              </div>
          </div>
          <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('pim.editor.name')}</label>
              <input className="w-full border p-2 rounded text-sm" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} />
          </div>
           <div className="grid grid-cols-2 gap-4">
              <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('pim.editor.category')}</label>
                  <input className="w-full border p-2 rounded text-sm" value={editForm.category} onChange={e => setEditForm({...editForm, category: e.target.value})} />
              </div>
               <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('pim.editor.status')}</label>
                  <select className="w-full border p-2 rounded text-sm" value={editForm.status} onChange={e => setEditForm({...editForm, status: e.target.value as ProductStatus})}>
           ...
           <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('pim.editor.tags')}</label>
              <input className="w-full border p-2 rounded text-sm" placeholder="New, Sale, ..." value={editForm.tags?.join(', ')} onChange={e => setEditForm({...editForm, tags: e.target.value.split(',').map(s => s.trim())})} />
          </div>
      </div>
  );
      ...
      <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
               <div>
                  ...
                  <input type="number" className="w-full border p-2 rounded text-sm" value={editForm.price} onChange={e => setEditForm({...editForm, price: parseFloat(e.target.value)})} />
              </div>
          </div>
          <div className="border-t pt-4">
              ...
              <div className="space-y-3">
                  <div className="flex items-center justify-between">
                      ...
                      <Toggle enabled={!!editForm.allowBackorder} onToggle={() => setEditForm({...editForm, allowBackorder: !editForm.allowBackorder})} />
                  </div>
                   <div className="flex items-center justify-between">
                      <span className="text-sm">{t('pim.editor.allow_transfer')}</span>
                      <Toggle enabled={!!editForm.allowTransfer} onToggle={() => setEditForm({...editForm, allowTransfer: !editForm.allowTransfer})} />
                  </div>
              </div>
          </div>
      </div>
               ...
               {editForm.mediaAssets?.map((media, idx) => (
                   <div key={idx} className="flex items-center gap-3 border p-2 rounded bg-white">
                       <div className="flex flex-col items-center gap-1">
                           <button onClick={() => moveMedia(idx, 'up')} disabled={idx === 0} className="text-gray-300 hover:text-gray-600 disabled:opacity-20"><ArrowUp size={14} /></button>
                           <button onClick={() => moveMedia(idx, 'down')} disabled={idx === (editForm.mediaAssets?.length || 0) - 1} className="text-gray-300 hover:text-gray-600 disabled:opacity-20"><ArrowDown size={14} /></button>
                       </div>
                       ...
                       <div className="flex items-center gap-2">
                           {!media.isMain && media.type === 'IMAGE' && (
                               <button onClick={() => setMainMedia(idx)} className="text-xs text-gray-400 hover:text-blue-600" title={t('pim.editor.set_main')}>
                                   <Star size={16} />
                               </button>
                           )}
