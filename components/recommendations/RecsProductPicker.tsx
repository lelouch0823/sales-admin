...
import { Product, Recommendation, StockStatus } from '../../types';
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-medium flex items-center gap-2">
                    ...
                    {isUnavailable && <span className="text-[10px] bg-danger-light text-danger px-1 rounded">{t('consts.stock.UNAVAILABLE')}</span>}
                  </div>
                  <div className="text-xs text-gray-500">{p.brand}</div>
                </div>
              </div>
              <Button
                variant={isAdded || isOffShelf || isUnavailable ? 'ghost' : 'primary'}
                size="sm"
