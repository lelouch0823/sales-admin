...
import { ClipboardList, PackagePlus, ArrowRightLeft, MapPin, Truck, Lock, PackageMinus } from 'lucide-react';
...
interface StockSkuCardProps {
    ...
    onSetActionModal: (action: { type: 'RECEIVE' | 'ISSUE' | 'TRANSFER' | 'RESERVE', sku: string, whId?: string }) => void;
    getStock: (sku: string, whId: string) => InventoryRecord | { onHand: number, reserved: number, inTransitIn: number, inTransitOut: number };
                    ...
                    {currentUserRole !== 'STORE_STAFF' && (
                        <div className="flex gap-2">
                            <Tooltip content={t('common.details')}>
                                <button onClick={() => onSetHistorySku(product.sku)} className="p-2 bg-white border border-gray-200 rounded hover:bg-gray-100 text-gray-600"><ClipboardList size={16} /></button>
                            </Tooltip>
                            <Tooltip content={t('inventory.restock')}>
                                <button onClick={() => onSetActionModal({ type: 'RECEIVE', sku: product.sku })} className="p-2 bg-white border border-gray-200 rounded hover:bg-gray-100 text-success"><PackagePlus size={16} /></button>
                            </Tooltip>
                            <Tooltip content={t('inventory.actions.transfer')}>
                                <button onClick={() => onSetActionModal({ type: 'TRANSFER', sku: product.sku })} className="p-2 bg-white border border-gray-200 rounded hover:bg-gray-100 text-brand"><ArrowRightLeft size={16} /></button>
                            </Tooltip>
                        </div>
                    )}
                </div>
                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                        {warehouses.map(wh => {
                            ...
                            else if (product.allowBackorder) { statusKey = 'BACKORDER'; variant = 'warning'; }
                            else if (product.allowTransfer) { statusKey = 'TRANSFERABLE'; variant = 'default'; }
                                        ...
                                        {(stock.onHand > 0 || currentUserRole === 'SUPER_ADMIN') && (
                                            <>
                                                <Tooltip content={t('inventory.actions.reserve')}>
                                                    <button
                                                        onClick={() => onSetActionModal({ type: 'RESERVE', sku: product.sku, whId: wh.id })}
                                                ...
                                                <Tooltip content={t('inventory.actions.issue')}>
                                                    <button
                                                        onClick={() => onSetActionModal({ type: 'ISSUE', sku: product.sku, whId: wh.id })}
