...
export const CustomerDetailPanel: React.FC<CustomerDetailPanelProps> = ({
  ...
  const [editForm, setEditForm] = useState<{ name: string; phone: string; tags: string }>({
    name: '',
    phone: '',
    tags: '',
  });
  ...
  const handleAddMember = (userId: string) => {
    ...
    toast.success(
      t('alerts.crm.shared_success', { role: t(`consts.shared_role.${selectedRole}`) })
    );
          ...
          <div className="flex items-center gap-2 shrink-0">
            {canEdit && !isEditing && (
              <button
                ...
                className="text-gray-400 hover:text-blue-600 p-2 rounded hover:bg-gray-50 transition-colors"
              >
                <Edit3 size={20} />
              </button>
            )}
            ...
            <button
              onClick={onClose}
              className="text-gray-400 hover:text-gray-600 p-2 rounded hover:bg-gray-50"
            >
              <X size={24} />
            </button>
          </div>
                  ...
                  <div key={share.userId} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">
                        ({t(`consts.shared_role.${share.role}`)})
                      </span>
                    </div>
                    {canShare && (
                      <button
              ...
              {(customer.interactions || []).map(interaction => {
                ...
                return (
                  <div key={interaction.id} className="relative">
                    <div
                      className={`absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full border-2 border-white ${interaction.type === 'NOTE' ? 'bg-blue-500' : 'bg-purple-500'}`}
                    ></div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-sm animate-in slide-in-from-bottom-2">
            ...
            <div className="space-y-2 mb-6 max-h-60 overflow-y-auto">
              {users
                .filter(
                  u =>
                    u.id !== customer.ownerUserId &&
                    !customer.sharedWith.some(s => s.userId === u.id)
                )
                .map(u => (
                  <div
                    key={u.id}
            ...
            <Button variant="ghost" fullWidth onClick={() => setIsShareModalOpen(false)}>
              {t('common.cancel')}
            </Button>
          </div>
        </div>
