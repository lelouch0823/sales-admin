...
interface CustomerRowProps {
    customer: Customer;
    currentUser: any; // Using any to avoid complex User type imports for now, or import properly
    isSelected: boolean;
    ...
    return (
        ...
        >
            <td className="py-4 px-6" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => onToggleSelection(customer.id)} className={`text-gray-400 hover:text-gray-600 ${isSelected ? 'text-brand' : ''}`}>
                ...
                <div className="flex items-center justify-end gap-2">
                    {!customer.ownerUserId && (
                        <Tooltip content={t('crm.actions.claim')}>
                            <button
                                onClick={() => { onClaim(customer.id, currentUser.id); toast.success(t('alerts.crm.claim_success')); }}
