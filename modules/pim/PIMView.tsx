...
export const PIMView: React.FC = () => {
  ...
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null); // 选中编辑的商品，null 表示新建
  ...
  const toggleAll = () => {
    if (selectedIds.size === filteredProducts.length) {
      ...
      setSelectedIds(new Set(filteredProducts.map(p => p.id))); // 全选当前页
    }
  };
  ...
  const handleCreate = () => {
      setSelectedProduct(null); // 清空选中项，进入新建模式
      setIsEditorOpen(true);
  };
      ...
      <Modal isOpen={isImportOpen} onClose={() => setIsImportOpen(false)} className="max-w-lg">
          <div className="p-6 text-center">
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-lg p-8 mb-4 bg-gray-50">
                    <button className="text-brand font-medium text-sm hover:underline">{t('pim.import_modal.download_template')}</button>
                </div>

                <div className="flex justify-end gap-3">
                    <button onClick={() => setIsImportOpen(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">{t('common.cancel')}</button>
                    <button onClick={() => { toast.info(t('alerts.pim.mock_import')); setIsImportOpen(false); }} className="px-4 py-2 text-sm bg-primary text-white rounded-lg shadow-sm hover:bg-primary-hover">{t('pim.import_modal.start')}</button>
                </div>
          </div>
      </Modal>
    </div>
