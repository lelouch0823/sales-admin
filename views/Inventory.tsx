...
import { Search, ArrowRightLeft, PackagePlus, PackageMinus, MapPin, History, ClipboardList, LayoutDashboard, TrendingUp, AlertTriangle, PackageX, Settings } from 'lucide-react';
...
export const InventoryView: React.FC = () => {
  ...
  // Modals
  const [actionModal, setActionModal] = useState<{type: 'RECEIVE'|'ISSUE'|'TRANSFER', sku?: string, whId?: string} | null>(null);
                         ...
                         <div key={m.id} className="flex items-center justify-between text-sm border-b border-gray-50 pb-2 last:border-0">
                             <div className="flex items-center gap-3">
                                 ...
                                 <div className={`w-8 h-8 rounded flex items-center justify-center 
                                     ...
                                     {m.type === 'RECEIVE' ? <PackagePlus size={16}/> : m.type === 'ISSUE' ? <PackageMinus size={16}/> : <ArrowRightLeft size={16}/>}
                                 </div>
                                 <div>
                                  ...
                                  {currentUser.role !== 'STORE_STAFF' && (
                                      <div className="flex gap-2">
                                           <button 
                                            ...
                                            className="p-2 bg-white border border-gray-200 rounded hover:bg-gray-100 text-gray-600" title={t('inventory.ledger')}
                                          >
                                              <ClipboardList size={16} />
                                          </button>
                                          <button 
                                            onClick={() => { setActionModal({type: 'RECEIVE', sku: p.sku}); }}
                                            className="p-2 bg-white border border-gray-200 rounded hover:bg-gray-100 text-success" title={t('inventory.actions.receive')}
                                          >
                                              <PackagePlus size={16} />
                                          </button>
                                          <button 
                                            onClick={() => { setActionModal({type: 'TRANSFER', sku: p.sku}); }}
                                            className="p-2 bg-white border border-gray-200 rounded hover:bg-gray-100 text-brand" title={t('inventory.actions.transfer')}
                                          >
                                                  ...
                                                  <td className="py-3 px-4 text-right">
                                                    {(stock.onHand > 0 || currentUser.role === 'SUPER_ADMIN') && (
                                                      <button 
                                                        ...
                                                        className="p-1 text-gray-400 hover:text-danger-text opacity-0 group-hover:opacity-100 transition-opacity" title={t('inventory.actions.issue')}
                                                      >
